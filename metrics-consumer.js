...
function processTweet(rawMessageValue) {
    ...
    if (tweet != undefined && tweet.filter) {
        var filter = tweet.filter;
        if (tweet.filter != currentFilter) {
            metrics = {};
            currentFilter = filter;
        }
        metrics.filter = filter;
        metrics.total_count = tweet.total_count;
        metrics.start_time = tweet.start_time;
    }
}
...
var reportInterval = setInterval(function () {
    ...
    if (number_of_connections > 0) {
        var jsonMessage = JSON.stringify(metrics);
